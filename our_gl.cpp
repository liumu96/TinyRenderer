...
void triangle(Vec3i *pts, IShader &shader, TGAImage &image, TGAImage &zbuffer)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 2; j++)
        {
            bboxmin[j] = std::min(bboxmin[j], (float)pts[i][j]);
            bboxmax[j] = std::max(bboxmax[j], (float)pts[i][j]);
        }
    }
    ...
    for (P.x = bboxmin.x; P.x <= bboxmax.x; P.x++)
    {
        ...
        for (P.y = bboxmin.y; P.y <= bboxmax.y; P.y++)
        {
            ...
            P.z = std::max(0, std::min(255, int(pts[0].z * c.x + pts[1].z * c.y + pts[2].z * c.z + .5)));
            if (c.x < 0 || c.y < 0 || c.z < 0 || zbuffer.get(P.x, P.y)[0] > P.z)
                continue;
